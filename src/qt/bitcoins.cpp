...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitcoinsUnit>("BitcoinsUnit");
#else
    qRegisterMetaType<BitcoinsUnit>("BitcoinsUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoins_de.qm (shortcut "de" needs to be defined in bitcoins.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoins_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoins.qrc)
...
static const char* qt_argv = "bitcoins-qt";

BitcoinsApplication::BitcoinsApplication()
...
void BitcoinsApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitcoinsApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcoinsGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitcoinsApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitcoinsApplication::createOptionsModel(bool resetSettings)
{
...
void BitcoinsApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcoinsGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitcoinsGUI::quitRequested, this, &BitcoinsApplication::requestShutdown);
...
void BitcoinsApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitcoinsApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitcoinsApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitcoinsApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitcoinsApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitcoinsApplication::handleRunawayException);
    connect(this, &BitcoinsApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitcoinsApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitcoinsApplication::parameterSetup()
{
...
void BitcoinsApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitcoinsApplication::requestInitialize()
{
...
void BitcoinsApplication::requestShutdown()
{
...
void BitcoinsApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // bitcoins: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitcoinsGUI::handlePaymentRequest);
            connect(window, &BitcoinsGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitcoinsApplication::handleRunawayException(const QString &message)
{
...
void BitcoinsApplication::handleNonFatalException(const QString& message)
{
...
WId BitcoinsApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitcoinsApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcoinsGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitcoins);
    Q_INIT_RESOURCE(bitcoins_locale);
    ...
    /// 6-7. Parse bitcoins.conf, determine network, switch to network specific
    ...
    // bitcoins: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
