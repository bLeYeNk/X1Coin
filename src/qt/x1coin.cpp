...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<X1coinUnit>("X1coinUnit");
#else
    qRegisterMetaType<X1coinUnit>("X1coinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. x1coin_de.qm (shortcut "de" needs to be defined in x1coin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. x1coin_de_DE.qm (shortcut "de_DE" needs to be defined in x1coin.qrc)
...
static const char* qt_argv = "x1coin-qt";

X1coinApplication::X1coinApplication()
...
void X1coinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the X1coinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", X1coinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void X1coinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool X1coinApplication::createOptionsModel(bool resetSettings)
{
...
void X1coinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new X1coinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &X1coinGUI::quitRequested, this, &X1coinApplication::requestShutdown);
...
void X1coinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void X1coinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool X1coinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void X1coinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &X1coinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &X1coinApplication::handleRunawayException);
    connect(this, &X1coinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &X1coinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void X1coinApplication::parameterSetup()
{
...
void X1coinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void X1coinApplication::requestInitialize()
{
...
void X1coinApplication::requestShutdown()
{
...
void X1coinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // x1coin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &X1coinGUI::handlePaymentRequest);
            connect(window, &X1coinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void X1coinApplication::handleRunawayException(const QString &message)
{
...
void X1coinApplication::handleNonFatalException(const QString& message)
{
...
WId X1coinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool X1coinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", X1coinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(x1coin);
    Q_INIT_RESOURCE(x1coin_locale);
    ...
    /// 6-7. Parse x1coin.conf, determine network, switch to network specific
    ...
    // x1coin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
