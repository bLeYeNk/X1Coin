...
const std::string X1coinGUI::DEFAULT_UIPLATFORM =
...
X1coinGUI::X1coinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &X1coinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &X1coinGUI::showModalOverlay);
...
void X1coinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a X1coin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and x1coin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &X1coinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &X1coinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &X1coinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your X1coin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified X1coin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed X1coin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed X1coin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a x1coin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible X1coin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &X1coinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &X1coinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &X1coinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &X1coinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &X1coinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &X1coinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &X1coinGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &X1coinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &X1coinGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &X1coinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &X1coinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &X1coinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &X1coinGUI::showDebugWindow);
}

void X1coinGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void X1coinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &X1coinGUI::setCurrentWalletBySelectorIndex);
...
void X1coinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &X1coinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &X1coinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &X1coinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &X1coinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void X1coinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void X1coinGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &X1coinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &X1coinGUI::removeWallet);
...
WalletController* X1coinGUI::getWalletController()
{
    return m_wallet_controller;
}

void X1coinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &X1coinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &X1coinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &X1coinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &X1coinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &X1coinGUI::incomingTransaction);
    connect(this, &X1coinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void X1coinGUI::removeWallet(WalletModel* walletModel)
{
...
void X1coinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void X1coinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void X1coinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void X1coinGUI::setWalletActionsEnabled(bool enabled)
{
...
void X1coinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &X1coinGUI::toggleHidden);
...
void X1coinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void X1coinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void X1coinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void X1coinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void X1coinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void X1coinGUI::openClicked()
{
...
void X1coinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void X1coinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void X1coinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void X1coinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void X1coinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void X1coinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void X1coinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void X1coinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to X1coin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void X1coinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void X1coinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void X1coinGUI::updateHeadersSyncProgressLabel()
{
...
void X1coinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void X1coinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &X1coinGUI::quitRequested);
...
void X1coinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void X1coinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void X1coinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &X1coinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &X1coinGUI::show);
                e->ignore();
            }
        }
    }
...
void X1coinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void X1coinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void X1coinGUI::incomingTransaction(const QString& date, X1coinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(X1coinUnits::formatWithUnit(unit, amount, true));
...
void X1coinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void X1coinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool X1coinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool X1coinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void X1coinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void X1coinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void X1coinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void X1coinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void X1coinGUI::updateWindowTitle()
{
...
void X1coinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void X1coinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void X1coinGUI::detectShutdown()
{
...
void X1coinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void X1coinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(X1coinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = X1coinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void X1coinGUI::subscribeToCoreSignals()
{
...
void X1coinGUI::unsubscribeFromCoreSignals()
{
...
bool X1coinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<X1coinUnit> units = X1coinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const X1coinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, X1coinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const X1coinUnit u : X1coinUnits::availableUnits()) {
        menu->addAction(X1coinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(X1coinUnit newUnits)
{
    setText(X1coinUnits::longName(newUnits));
}
