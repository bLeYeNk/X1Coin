...
Tracing (USDT) support for X1coin on Linux using. For more information on
USDT support in X1coin see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `x1coind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `x1coind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[X1coin USDT documentation] on how to list available tracepoints in your
`x1coind` binary.

[X1coin USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/x1coind
```
...
X1coin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/x1coind
```
...
In a different terminal, starting X1coin in SigNet mode and with
re-indexing enabled.

```
$ ./src/x1coind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/x1coind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/x1coind
```
