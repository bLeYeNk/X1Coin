...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/x1coin.conf are parsed in qt/x1coin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  x1coind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see x1coind -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for x1coind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect x1coind signal handlers
    noui_connect();
