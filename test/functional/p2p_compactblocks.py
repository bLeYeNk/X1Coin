...
# TestP2PConn: A peer we use to send messages to x1coind, and store responses.
...
class CompactBlocksTest(X1coinTestFramework):
    ...
    # This test actually causes x1coind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that x1coind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for x1coind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # x1coind will not send blocktxn responses for blocks whose height is
