...
"""Test x1coind with different proxy configuration.

Test plan:
- Start x1coind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on x1coind side:
...
class ProxyTest(X1coinTestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: x1coind's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
