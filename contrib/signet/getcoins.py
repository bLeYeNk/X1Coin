...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing x1coin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='x1coin-cli', help='x1coin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='X1coin address to which the faucet should send')
...
parser.add_argument('x1coin_cli_args', nargs='*', help='Arguments to pass on to x1coin-cli (default: -signet)')

args = parser.parse_args()

if args.x1coin_cli_args == []:
    args.x1coin_cli_args = ['-signet']


def x1coin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.x1coin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = x1coin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = x1coin_cli(['getnewaddress', 'faucet', 'bech32'])
