...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoins-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoins-cli', help='bitcoins-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bitcoins address to which the faucet should send')
...
parser.add_argument('bitcoins_cli_args', nargs='*', help='Arguments to pass on to bitcoins-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoins_cli_args == []:
    args.bitcoins_cli_args = ['-signet']


def bitcoins_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoins_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoins_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoins_cli(['getnewaddress', 'faucet', 'bech32'])
