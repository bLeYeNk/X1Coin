...
const std::string BitcoinsGUI::DEFAULT_UIPLATFORM =
...
BitcoinsGUI::BitcoinsGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitcoinsGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitcoinsGUI::showModalOverlay);
...
void BitcoinsGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Bitcoins address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitcoins: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BitcoinsGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitcoinsGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitcoinsGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Bitcoins addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Bitcoins addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Bitcoins Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Bitcoins Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bitcoins: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bitcoins command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BitcoinsGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitcoinsGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitcoinsGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitcoinsGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitcoinsGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitcoinsGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitcoinsGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitcoinsGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &BitcoinsGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BitcoinsGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BitcoinsGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitcoinsGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitcoinsGUI::showDebugWindow);
}

void BitcoinsGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void BitcoinsGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitcoinsGUI::setCurrentWalletBySelectorIndex);
...
void BitcoinsGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitcoinsGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitcoinsGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitcoinsGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitcoinsGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitcoinsGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void BitcoinsGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitcoinsGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitcoinsGUI::removeWallet);
...
WalletController* BitcoinsGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitcoinsGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitcoinsGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitcoinsGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitcoinsGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitcoinsGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitcoinsGUI::incomingTransaction);
    connect(this, &BitcoinsGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BitcoinsGUI::removeWallet(WalletModel* walletModel)
{
...
void BitcoinsGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BitcoinsGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitcoinsGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitcoinsGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitcoinsGUI::createTrayIcon()
{
...
void BitcoinsGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitcoinsGUI::toggleHidden);
...
void BitcoinsGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitcoinsGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitcoinsGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitcoinsGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitcoinsGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitcoinsGUI::openClicked()
{
...
void BitcoinsGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitcoinsGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitcoinsGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitcoinsGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitcoinsGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitcoinsGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitcoinsGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitcoinsGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bitcoins network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitcoinsGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitcoinsGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitcoinsGUI::updateHeadersSyncProgressLabel()
{
...
void BitcoinsGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BitcoinsGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitcoinsGUI::quitRequested);
...
void BitcoinsGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitcoinsGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitcoinsGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitcoinsGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitcoinsGUI::show);
                e->ignore();
            }
        }
    }
...
void BitcoinsGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitcoinsGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitcoinsGUI::incomingTransaction(const QString& date, BitcoinsUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitcoinsUnits::formatWithUnit(unit, amount, true));
...
void BitcoinsGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitcoinsGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitcoinsGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitcoinsGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitcoinsGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitcoinsGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitcoinsGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitcoinsGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BitcoinsGUI::updateWindowTitle()
{
...
void BitcoinsGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitcoinsGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitcoinsGUI::detectShutdown()
{
...
void BitcoinsGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitcoinsGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitcoinsGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitcoinsGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitcoinsGUI::subscribeToCoreSignals()
{
...
void BitcoinsGUI::unsubscribeFromCoreSignals()
{
...
bool BitcoinsGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitcoinsUnit> units = BitcoinsUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitcoinsUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitcoinsUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitcoinsUnit u : BitcoinsUnits::availableUnits()) {
        menu->addAction(BitcoinsUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitcoinsUnit newUnits)
{
    setText(BitcoinsUnits::longName(newUnits));
}
