...
# The block reward of coinbaseoutput.nValue (50) X1/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 X1/block.
...
class TestX1coinCli(X1coinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `x1coin-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and x1coin-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and x1coin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (X1/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (X1/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 X1) to the default wallet.
            ...
            # Test x1coin-cli -generate with -rpcwallet in multiwallet mode.
